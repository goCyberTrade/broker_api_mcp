...
import com.ebang.openapi.enums.FutuConnStatus;
import com.ebang.openapi.exception.OpenApiErrorCodeEnums;
import com.ebang.openapi.exception.OpenApiException;
import com.ebang.openapi.req.CashFlowSummaryReq;
import com.ebang.openapi.req.MaximumTradableQuantityReq;
import com.ebang.openapi.req.MarginTradingDataReq;
import com.ebang.openapi.req.PositionListQueryReq;
...
public class FutuBase implements FTSPI_Conn, FTSPI_Qot, FTSPI_Trd {
    ...
    public FutuConnStatus qotFutuConnStatus = FutuConnStatus.DISCONNECT;
    public FutuConnStatus trdFutuConnStatus = FutuConnStatus.DISCONNECT;
    ...
    public void onInitConnect(FTAPI_Conn client, long errCode, String desc) {
        if (client instanceof FTAPI_Conn_Qot) {
            synchronized (qotLock) {
                if (errCode == 0) {
                    qotFutuConnStatus = FutuConnStatus.READY;
                }
                qotLock.notifyAll();
                return;
            }
        ...
        if (client instanceof FTAPI_Conn_Trd) {
            synchronized (trdLock) {
                if (errCode == 0) {
                    trdFutuConnStatus = FutuConnStatus.READY;
                }
                trdLock.notifyAll();
                return;
            }
    ...
    public void onDisconnect(FTAPI_Conn client, long errCode) {
        if (client instanceof FTAPI_Conn_Qot) {
            synchronized (qotLock) {
                qotFutuConnStatus = FutuConnStatus.DISCONNECT;
                return;
            }
        }
        ...
        if (client instanceof FTAPI_Conn_Trd) {
            synchronized (trdLock) {
                trdFutuConnStatus = FutuConnStatus.DISCONNECT;
            }
        }
    }

    public void initConnect(Map<String, String> routeConfig) {
        // 验证配置是否存在
        try {
            if (trdFutuConnStatus != FutuConnStatus.READY) {
                if (!initConnectTrdSync(routeConfig.get("opendIP"), Short.valueOf(routeConfig.get("opendPort")))) {
                    throw new OpenApiException(OpenApiErrorCodeEnums.INVALID_CALL_ROUTE.getErrorCode(), "trade server connect error");
                }
                else {
                    // 解锁交易
                    unlockTradeSync(MD5Util.calcMD5(routeConfig.get("tradePassword")), true);
                }
                if (!initConnectQotSync(routeConfig.get("opendIP"), Short.valueOf(routeConfig.get("opendPort")))) {
                    throw new OpenApiException(OpenApiErrorCodeEnums.INVALID_CALL_ROUTE.getErrorCode(), "quote server connect error");
                }
            }
        } catch (Exception e) {
            log.error("连接富途发生异常", e);
            throw new OpenApiException(OpenApiErrorCodeEnums.INVALID_CALL_ROUTE.getErrorCode(), e.getMessage());
        }
    }

    public boolean initConnectQotSync(String ip, short port) throws InterruptedException {
        qot.setConnSpi(this);
        qot.setQotSpi(this);
        synchronized (qotLock) {
            ...
            if (!ret)
                return false;
            qotLock.wait();
            return qotFutuConnStatus == FutuConnStatus.READY;
        }
    }

    public boolean initConnectTrdSync(String ip, short port) throws InterruptedException {
        trd.setConnSpi(this);
        trd.setTrdSpi(this);
        synchronized (trdLock) {
            ...
            if (!ret)
                return false;
            trdLock.wait();
            return trdFutuConnStatus == FutuConnStatus.READY;
        }
    }
    ...
    QotGetSecuritySnapshot.Response getSecuritySnapshotSync(List<QotCommon.Security> secList) throws InterruptedException {
        ...
        synchronized (syncEvent) {
            synchronized (qotLock) {
                if (qotFutuConnStatus != FutuConnStatus.READY)
                    return null;
    ...
    public TrdModifyOrder.Response modifyOrderSync(TrdModifyOrder.C2S.Builder c2s) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                if (trdFutuConnStatus != FutuConnStatus.READY)
                    return null;
                c2s.setPacketID(trd.nextPacketID());
                TrdModifyOrder.Request req = TrdModifyOrder.Request.newBuilder().setC2S(c2s).build();
                int sn = trd.modifyOrder(req);
                if (sn == 0)
                    return null;
                reqInfo = new ReqInfo(ProtoID.TRD_MODIFYORDER, syncEvent);
                trdReqInfoMap.put(sn, reqInfo);
            }
            syncEvent.wait();
            return (TrdModifyOrder.Response)reqInfo.rsp;
        }
    }

    public TrdPlaceOrder.Response placeOrderSync(TrdPlaceOrder.C2S.Builder c2s) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
    ...
    public TrdGetOrderFee.Response getOrderFee(TrdGetOrderFee.C2S.Builder c2s) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                if (sn == 0)
                    return null;
                reqInfo = new ReqInfo(ProtoID.TRD_GETORDERFEE, syncEvent);
                trdReqInfoMap.put(sn, reqInfo);
            }
            syncEvent.wait();
            return (TrdGetOrderFee.Response)reqInfo.rsp;
        }
    }
    public TrdUnlockTrade.Response unlockTradeSync(String pwdMD5, boolean isUnlock) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
    ...
    public TrdGetAccList.Response getAccListSync() throws InterruptedException {
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                if (trdFutuConnStatus != FutuConnStatus.READY)
                    return null;
                //userId 历史原因，目前已废弃，填0即可
                TrdGetAccList.C2S c2s = TrdGetAccList.C2S.newBuilder().setUserID(0L).build();
    ...
    public TrdGetOrderList.Response getOrderListSync(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                     boolean isRefreshCache,
    ...
    public TrdGetHistoryOrderList.Response getHistoryOrderListSync(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                            TrdCommon.TrdFilterConditions filterConditions,
                                                            List<TrdCommon.OrderStatus> filterStatusList) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
    ...
    public TrdGetHistoryOrderFillList.Response getHistoryOrderFillListSync(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                                    TrdCommon.TrdFilterConditions filterConditions) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
    ...
    public TrdGetMaxTrdQtys.Response getMaxTrdQtys(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                   MaximumTradableQuantityReq request) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        ...
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                TrdGetMaxTrdQtys.C2S c2s = TrdGetMaxTrdQtys.C2S.newBuilder()
                        .setHeader(trdHeader)
                        .setOrderType(Integer.parseInt(request.getFutuOrderType()))
                        .setCode(request.getCode())
                        .setPrice(request.getPrice())
                        .setOrderID(Long.parseLong(request.getOrderId()))
                        .setAdjustPrice(request.isAdjustPrice())
                        .setAdjustSideAndLimit(request.getAdjustSideAndLimit())
                        .setSecMarket(request.getSecMarket())
                        .setOrderIDEx(request.getOrderIDEx())
                        .build();
                TrdGetMaxTrdQtys.Request req = TrdGetMaxTrdQtys.Request.newBuilder().setC2S(c2s).build();
                int sn = trd.getMaxTrdQtys(req);
                if (sn == 0) {
                    return null;
                }
                reqInfo = new ReqInfo(ProtoID.TRD_GETMAXTRDQTYS, syncEvent);
                trdReqInfoMap.put(sn, reqInfo);
            }
            syncEvent.wait();
            return (TrdGetMaxTrdQtys.Response)reqInfo.rsp;
        }
    }

    public TrdGetMaxTrdQtys.Response getMarginRatio(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                    MarginTradingDataReq request) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                TrdCommon.TrdHeader trdHeader = makeTrdHeader(trdEnv, accID, trdMarket);
                QotCommon.Security security = makeSec(QotCommon.QotMarket.forNumber(request.getMarket()), request.getCode());
                ...
                reqInfo = new ReqInfo(ProtoID.TRD_GETMARGINRATIO, syncEvent);
                trdReqInfoMap.put(sn, reqInfo);
            }
            syncEvent.wait();
            return (TrdGetMaxTrdQtys.Response)reqInfo.rsp;
    ...
    public TrdFlowSummary.Response getFlowSummary(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                  CashFlowSummaryReq request) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                TrdFlowSummary.C2S c2s = TrdFlowSummary.C2S.newBuilder()
                        .setHeader(trdHeader)
                        .setClearingDate(request.getClearingDate())
                        .setCashFlowDirection(request.getCashFlowDirection())
                        .build();
                TrdFlowSummary.Request req = TrdFlowSummary.Request.newBuilder().setC2S(c2s).build();
                int sn = trd.getFlowSummary(req);
                if (sn == 0) {
                    return null;
                }
                reqInfo = new ReqInfo(ProtoID.TRD_GETFLOWSUMMARY, syncEvent);
                trdReqInfoMap.put(sn, reqInfo);
            }
            syncEvent.wait();
            return (TrdFlowSummary.Response)reqInfo.rsp;
        }
    }

    public TrdGetPositionList.Response getPositionList(long accID, TrdCommon.TrdMarket trdMarket, TrdCommon.TrdEnv trdEnv,
                                                  PositionListQueryReq request) throws InterruptedException {
        ReqInfo reqInfo = null;
        Object syncEvent = new Object();
        synchronized (syncEvent) {
            synchronized (trdLock) {
                ...
                TrdCommon.TrdFilterConditions conditions = buildCondition(request.getSymbol(),
                        request.getPositionId(), request.getTrdMarket(), request.getStartDate(), request.getEndDate());
    ...
    public TrdCommon.TrdFilterConditions buildCondition(
            String symbol,
            String idList,
            Integer trdMarket,
            String beginTime,
        ...
        if (StringUtils.isNotBlank(symbol)) {
            builder.addAllCodeList(Stream.of(symbol.split(",")).distinct().collect(Collectors.toList()));
        }
        if (StringUtils.isNotBlank(idList)) {
            builder.addAllIdList(Stream.of(idList.split(",")).map(Long::parseLong).distinct().collect(Collectors.toList()));
        }
        if (trdMarket != null) {
            builder.setFilterMarket(trdMarket);
        }
        if (beginTime != null) {
